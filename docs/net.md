...
- `(:type :snapshot :state <game-state> :events (<event> ...))` -> server snapshot + HUD/combat events

Key functions
- `run-server`: bind UDP on `*net-default-host*`/`*net-default-port*`, apply intents, tick `server-step`, send snapshots.
...
- `apply-snapshot`: apply a snapshot via `apply-game-state` and queue HUD/combat events.
