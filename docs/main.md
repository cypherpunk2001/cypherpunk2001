...
- `handle-window-resize`: checks for window resize events and updates UI layout and camera offset when `*window-resize-enabled*` is T. Called each frame in both `run-local` and `run-client` loops.
- `npc-array-for-player-zone`: returns the correct NPC array for a player's zone (zone-state NPCs when available, falls back to game-npcs). Used in input processing and simulation sync loops to ensure consistent NPC lookups.
