;;;; scripts/test-unit.lisp
;;;; Run from repo root:
;;;;   sbcl --script scripts/test-unit.lisp
...
(handler-case
    (progn
      ...
      ;; 4) Load test file
      (load (merge-pathnames "tests/unit-test.lisp"))

      ;; 5) Run tests
      (let ((success (funcall (read-from-string "mmorpg::run-unit-tests"))))
        (format t "~%")
        (if success
            (progn
              (format t "OK: All unit tests passed~%")
              (sb-ext:exit :code 0))
            (progn
              (format t "FAILED: Some unit tests failed~%")
              (sb-ext:exit :code 1)))))
  (error (e)
    (die 1 "TEST-UNIT ERROR: ~a" e)))
