...
(defun draw-world (world render assets camera player npcs ui editor)
  ...
  (let* ((tile-dest-size (world-tile-dest-size world))
         ...
         ;; Use editor-camera-target for consistent camera focus (includes leash)
         (x (multiple-value-bind (cx cy) (editor-camera-target editor player camera)
              (declare (ignore cy)) cx))
         (y (multiple-value-bind (cx cy) (editor-camera-target editor player camera)
    ...
    (when *debug-collision-overlay*
      ...
      (let ((ix (round (- (player-x player) (world-collision-half-width world))))
            (iy (round (- (player-y player) (world-collision-half-height world))))
